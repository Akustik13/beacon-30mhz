...
import threading, queue, re, datetime, os, json, time
...
# Mapping: first word of UART command → expected ACK substring in MCU response.
# Empty string → accept first non-empty, non-error line as ACK.
_ACK_MAP = {
    "mode":   "[MODE]",
    "ch":     "[CH]",
    "pwr":    "[PWR]",
    ...
    "led":    "[LED]",
    "save":   "saved OK",
    "reset":  "",           # MCU reboots — no reliable ACK; will timeout (that's fine)
    "sleep":  "",
    "tx":     "[TX",
    "sched":  "[SCHED]",
    "log":    "[LOG]",
...
def _ack_for_cmd(cmd: str) -> str:
    """Return expected ACK substring for a UART command.
    Empty string = accept first non-empty, non-error line.
    Sub-commands for temp/batt/light/rtc are narrowed to avoid
    racing with periodic sensor lines."""
    ...
    if w == "temp":
        return {"mode":   "[TEMP] mode=",
                "period": "[TEMP] period=",
                "offset": "[TEMP] offset=",
                "read":   "[TEMP] chip="}.get(s, "[TEMP]")
    if w == "batt":
        return {"mode":   "[BATT] mode=",
                "period": "[BATT] period=",
                "scale":  "[BATT] scale=",
                "read":   "Battery:"}.get(s, "[BATT]")
    if w == "light":
        return {"mode":   "[LIGHT] mode=",
                "period": "[LIGHT] period=",
                "read":   "[LIGHT]"}.get(s, "[LIGHT]")
    if w == "rtc":
        return {"live": "[RTC] live=", "set": "[RTC]",
                "get":  "[RTC]"}.get(s, "[RTC]")
    return _ACK_MAP.get(w, "")


def _cmd_to_ind(cmd: str) -> str:
    """Map command string → indicator name key."""
    parts = cmd.strip().lower().split()
    if not parts: return ""
    w, s = parts[0], (parts[1] if len(parts) > 1 else "")
    if w == "temp":
        return {"mode": "temp_mode", "period": "temp_period",
                "offset": "temp_offset"}.get(s, "")
    if w == "batt":
        return {"mode": "batt_mode", "period": "batt_period",
                "scale":  "batt_scale"}.get(s, "")
    if w == "light":
        return {"mode": "light_mode", "period": "light_period"}.get(s, "")
    if w == "log":
        return {"mask": "log_mask", "mode": "log_mode", "overflow": "log_oflow",
                "ckp":  "log_ckp",  "ts":   "log_ts",
                "temp": "log_temp", "light":"log_light", "bat": "log_bat"}.get(s, "")
    if w == "sched": return "sched"
    if w == "save":  return "save"
    if w == "rtc":   return "rtc_live"
    return {"mode": "mode", "ch": "ch", "pwr": "pwr",
            "pulse": "pulse", "period": "period", "led": "led"}.get(w, "")


class CmdQueue:
    """Sends commands one at a time. Waits for MCU ACK tag (or timeout) before sending next."""
    TIMEOUT_MS = 1500

    def __init__(self, gui):
        self._gui          = gui
        self._queue        = []
        self._current      = None   # (cmd, desc) in flight
        self._results      = []     # list of (cmd, desc, status)
        self._timeout_id   = None
        self._on_done      = None
        self._last_ram_idx = -1     # index of most-recent OK_RAM result (for flash upgrade)

    def run(self, commands, on_done=None):
        """commands: list of (cmd_str, description). on_done: callback(results)."""
        self._queue              = list(commands)
        self._results            = []
        self._last_ram_idx       = -1
        self._on_done            = on_done
        self._gui._last_ind_cmd  = None  # cancel any pending individual-cmd tracking
        self._send_next()
    ...
    def _send_next(self):
        ...
        cmd, _desc = self._current
        self._gui._cmd_raw(cmd)
        self._timeout_id = self._gui.after(self.TIMEOUT_MS, self._on_timeout)
    ...
    def on_line(self, line: str):
        ...
        if not stripped:
            return
        # Flash confirmation: upgrade most-recent RAM-only result to FLASH
        if "[CFG] auto-saved" in stripped:
            if 0 <= self._last_ram_idx < len(self._results):
                c, d, s = self._results[self._last_ram_idx]
                if s == 'OK_RAM':
                    self._results[self._last_ram_idx] = (c, d, 'OK_FLASH')
                    self._gui._set_indicator(_cmd_to_ind(c), 'flash')
                    self._last_ram_idx = -1
        if not self._current:
            return
        cmd, desc = self._current
        ack    = _ack_for_cmd(cmd)
        is_err = stripped.startswith('[ERR]') or stripped.startswith('ERR')
        ack_hit = (ack != "" and ack in stripped) or (ack == "" and not is_err)
        if ack_hit:
            if "saved OK" in stripped:
                # explicit `save` command — flash confirmed directly by its ACK line
                self._results.append((cmd, desc, 'OK_FLASH'))
                self._gui._set_indicator(_cmd_to_ind(cmd), 'flash')
                self._last_ram_idx = -1
            else:
                self._results.append((cmd, desc, 'OK_RAM'))
                self._last_ram_idx = len(self._results) - 1
                self._gui._set_indicator(_cmd_to_ind(cmd), 'ram')
            self._current = None
            self._send_next()
        elif is_err:
            self._results.append((cmd, desc, stripped[:60]))
            self._gui._set_indicator(_cmd_to_ind(cmd), 'fail')
            self._last_ram_idx = -1
            self._current = None
            self._send_next()
...
class TxBeaconGUI(tk.Tk):
    def __init__(self):
        ...
        self.rx_thread = None
        self._cmd_queue    = CmdQueue(self)
        self._param_ind    = {}   # name → tk.Label indicator widget
        self._last_ind_cmd = None # last individually-sent cmd (for indicator update)
        ...
        # Flash log tab
        self._log_total       = 768          # updated when MCU reports actual capacity
        ...
        self._log_iv_temp_var  = tk.StringVar(value="60")
        self._log_iv_light_var = tk.StringVar(value="300")
        self._log_iv_bat_var   = tk.StringVar(value="3600")
        ...
        self._calc_hist_var    = tk.StringVar(value="")
        self._calc_warn_var    = tk.StringVar(value="")
        self._calc_parsing     = False   # True while receiving [LOG] calc output
    ...
    def _build_conn_bar(self):
        ...
        self._var_silent = tk.BooleanVar(value=False)
        self._chk_silent_cb = ttk.Checkbutton(
            f, text="Silent", variable=self._var_silent,
            style="TCheckbutton")
        self._chk_silent_cb.pack(side="left", padx=(16, 2))
        tk.Label(f, text="(suppress AT telemetry)", bg=BG3, fg="#64748b",
    ...
    def _set_indicator(self, name: str, state: str):
        lbl = self._param_ind.get(name)
        if not lbl:
            return
        sym, col = self._IND_STYLE.get(state, ("○", "#c9cdd4"))
        lbl.config(text=sym, fg=col)

    def _ind_lbl(self, parent, name, **pack_kw):
        """Create, register and return a parameter status indicator label."""
        lbl = tk.Label(parent, text="○", fg="#c9cdd4", bg=BG,
                       font=("Segoe UI", 10), cursor="arrow")
        self._param_ind[name] = lbl
        return lbl
    ...
    def _build_transmitter_panel(self, outer, row):
        ...
        self._ind_lbl(mf, 'mode').pack(side="left", padx=(8, 0))
        ...
        self._ind_lbl(cf, 'ch').pack(side="left", padx=(8, 0))
        ...
        self._ind_lbl(pf2, 'pwr').pack(side="left", padx=(8, 0))
        ...
        for r, (ind_name, lbl, var, lo, hi, cmd_fn) in enumerate([
            ("pulse",  "Pulse (ms):",  self._pulse_var,  1,       60000,
             lambda: self._cmd(f"pulse {self._pulse_var.get()}")),
            ("period", "Period (ms):", self._period_var, 100, 3600000,
            ...
            self._ind_lbl(tf, ind_name).grid(row=r, column=3, padx=(6, 0), sticky="w")
    ...
    def _build_led_panel(self, outer, row):
        ...
        self._ind_lbl(lf, 'led').grid(row=0, column=4, padx=(8, 0), sticky="w")
    ...
    def _build_temp_panel(self, outer, row):
        ...
        self._ind_lbl(mf, 'temp_mode').pack(side="left", padx=(8, 0))
        ...
        self._ind_lbl(po_f, 'temp_period').pack(side="left", padx=(0, 12))
        ...
        self._ind_lbl(po_f, 'temp_offset').pack(side="left")
    ...
    def _build_batt_panel(self, outer, row):
        ...
        self._ind_lbl(mf, 'batt_mode').pack(side="left", padx=(8, 0))
        ...
        self._ind_lbl(pf, 'batt_period').pack(side="left", padx=(0, 12))
        ...
        self._ind_lbl(pf, 'batt_scale').pack(side="left")
    ...
    def _build_light_panel(self, outer, row):
        ...
        self._ind_lbl(mf, 'light_mode').pack(side="left", padx=(8, 0))
        ...
        self._ind_lbl(pf, 'light_period').pack(side="left")
    ...
    def _build_actions_panel(self, outer, row):
        ...
        ttk.Button(af, text="💾   SAVE TO FLASH  &  RESTART MCU", style="Save.TButton",
                   ...
                   ).grid(row=3, column=0, columnspan=4, padx=3, pady=(4, 1), sticky="ew")
        save_row = tk.Frame(af, bg=BG); save_row.grid(row=4, column=0, columnspan=4, sticky="w", pady=(0, 3))
        self._ind_lbl(save_row, 'save').pack(side="left", padx=(4, 4))
        tk.Label(save_row, text="Flash write", bg=BG, fg=FG_DIM, font=("Segoe UI", 7)).pack(side="left")
    ...
    def _build_flash_log_panel(self, parent):
        ...
        self._ind_lbl(mf, 'log_mask').pack(side="left", padx=(4, 0))
        ...
        tk.Label(ivf, text="Sync", bg=BG, fg=FG_DIM,
                 font=("Segoe UI", 8)).grid(row=0, column=9, padx=(6, 0))
        # rows — store widget refs for state management
        _irows = [
            ("Temp",  "temp",  self._temp_period_var,  "temp period",  self._log_iv_temp_var, "log temp"),
            ("Light", "light", self._light_period_var, "light period", self._log_iv_light_var, "log light"),
            ("Bat",   "bat",   self._batt_period_var,  "batt period",  self._log_iv_bat_var, "log bat"),
        ]
        for r, (lbl, key, rv, rc, wv, wc) in enumerate(_irows, start=1):
            ...
            w_btn.grid(row=r, column=7, padx=(0, 2))
            self._ind_lbl(ivf, f"log_{key}").grid(row=r, column=8, padx=(2, 4))
            sync_cb = tk.Checkbutton(ivf, text="⇌", variable=self._log_sync_vars[key],
                                     bg=BG, fg=FG_DIM, selectcolor=BG2, activebackground=BG,
                                     font=("Segoe UI", 9))
            sync_cb.grid(row=r, column=9, padx=(4, 0))
        ...
        self._ind_lbl(mof, 'log_mode').pack(side="left", padx=(0, 8))
        ...
        self._ind_lbl(mof, 'log_oflow').pack(side="left", padx=(0, 8))
        ...
        self._ind_lbl(mof, 'log_ckp').pack(side="left")
        ...
        self._ind_lbl(tsf, 'log_ts').pack(side="left")
    ...
    def _build_calc_panel(self, parent):
        ...
        # ── Row 5: history depth (circular mode) / warning ───────────────────
        r4b = tk.Frame(cf, bg=BG); r4b.grid(row=5, column=0, sticky="w", pady=(2, 0))
        tk.Label(r4b, textvariable=self._calc_hist_var,
                 bg=BG, fg="#a78bfa", font=("Segoe UI", 8, "italic")).pack(side="left")
        tk.Label(r4b, textvariable=self._calc_warn_var,
                 bg=BG, fg="#f59e0b", font=("Segoe UI", 8, "italic")).pack(side="left", padx=(8, 0))

        # ── Row 6: separator / reverse ────────────────────────────────────────
        tk.Frame(cf, bg=BG3, height=1).grid(row=6, column=0, sticky="ew", pady=(6, 4))

        r5 = tk.Frame(cf, bg=BG); r5.grid(row=7, column=0, sticky="w")
        ...
        # ── Row 8: calculate button ───────────────────────────────────────────
        r7 = tk.Frame(cf, bg=BG); r7.grid(row=8, column=0, sticky="w", pady=(6, 0))
    ...
    def _calc_import(self):
        self._calc_total_var.set(str(self._log_total))
        try:    free = int(self._log_sv_free.get())
        except: free = self._log_total
        self._calc_free_var.set(str(min(free, self._log_total)))
        self._calc_temp_en.set(self._log_mask_temp.get())
        self._calc_temp_iv.set(self._log_iv_temp_var.get())
        self._calc_light_en.set(self._log_mask_light.get())
        self._calc_light_iv.set(self._log_iv_light_var.get())
        self._calc_bat_en.set(self._log_mask_batp.get())
        self._calc_bat_iv.set(self._log_iv_bat_var.get())
    ...
    def _calc_run(self):
        try:
            ...
            if mode == "always":
                rph = rph_raw
            elif mode == "adaptive":                          # 10b: checkpoint floor
                ckp_n = max(1, int(self._log_ckp_var.get() or 16))
                rph   = max(rph_raw * eff, rph_raw / ckp_n)
            else:
                rph = rph_raw * eff

            # 10d: warn if sensor intervals are not multiples of the fastest
            if len(sensors) > 1:
                base = min(sensors)
                if any(iv % base != 0 for iv in sensors):
                    self._calc_warn_var.set(
                        "⚠ intervals not multiples of fastest — actual rate may be higher")
                else:
                    self._calc_warn_var.set("")
            else:
                self._calc_warn_var.set("")
            ...
            # 10c: history depth for circular overflow mode
            if self._log_oflow_var.get() == "circular" and rph > 0:
                hist_h = total / rph
            ...
            if budget_rph > 0:
                eff_used = eff if mode != "always" else 1.0   # 10a: no compression for always
    ...
    def _parse_log_line(self, s: str):
        # ── [LOG] calc output ─────────────────────────────────────────────────
        if "Capacity Calculator" in s:
            self._calc_parsing = True
            # reset sensor enables; they'll be set only for sensors that appear in output
            self._calc_temp_en.set(False)
            self._calc_light_en.set(False)
            self._calc_bat_en.set(False)
            # sync write-mode from config tab
            self._calc_mode_var.set(self._log_mode_var.get())
            return
        if self._calc_parsing:
            m2 = re.search(r"\[LOG\]\s+Free entries:\s*(\d+)", s)
            if m2:
                self._calc_free_var.set(m2.group(1)); return
            m2 = re.search(r"\[LOG\]\s+TEMP\s+\((\d+)s\)", s)
            if m2:
                self._calc_temp_en.set(True)
                self._calc_temp_iv.set(m2.group(1)); return
            m2 = re.search(r"\[LOG\]\s+LIGHT\s+\((\d+)s\)", s)
            if m2:
                self._calc_light_en.set(True)
                self._calc_light_iv.set(m2.group(1)); return
            m2 = re.search(r"\[LOG\]\s+BAT\s+\((\d+)s\)", s)
            if m2:
                self._calc_bat_en.set(True)
                self._calc_bat_iv.set(m2.group(1)); return
            m2 = re.search(r"\[LOG\]\s+Total:\s+(\d+)/hr", s)
            if m2:
                self._calc_parsing = False
                self._calc_run()
                return
            return   # skip header/separator/footer lines during capture
        # ── normal [LOG] line parsing ─────────────────────────────────────────
        m = re.search(r"\[LOG\]\s+Used:\s+(\d+)\s+entries.*?(\d+)%", s)
        if m:
            used, pct = int(m.group(1)), int(m.group(2))
            self._log_sv_used.set(f"{used}/{self._log_total} ({pct}%)")
        ...
        if m:
            ...
            self._log_mode_var.set({"ALWAYS": "always", "ON_CHANGE": "onchange",
                                    "ADAPTIVE": "adaptive"}.get(v, "always"))
            self._set_indicator('log_mode', 'flash'); return
        m = re.search(r"\[LOG\]\s+Oflow:\s+(\S+)", s)
        if m:
            v = m.group(1).strip()
            self._log_sv_oflow.set(v)
            self._log_oflow_var.set("circular" if v == "CIRCULAR" else "stop")
            self._set_indicator('log_oflow', 'flash'); return
        ...
        if m:
            ...
            if m.group(5) is not None:
                ...
                self._set_indicator('log_ts', 'flash')
            ...
            self._set_indicator('log_mask',  'flash')
            self._set_indicator('log_oflow', 'flash')
            self._set_indicator('log_mode',  'flash')
            self._set_indicator('log_ckp',   'flash')
            return
        m = re.search(r"\[LOG\]\s+TS:\s+(\S+)", s)
        if m:
            self._log_ts_var.set("rtc" if "rtc" in m.group(1).lower() else "boot")
            self._set_indicator('log_ts', 'flash'); return
        m = re.search(r"\[LOG\]\s+ts=(rtc|boot)", s)
        if m:
            self._log_ts_var.set(m.group(1))
            self._set_indicator('log_ts', 'flash'); return
        m = re.search(r"\[LOG\]\s+temp=(\d+)s\s+light=(\d+)s\s+bat=(\d+)s", s)
        if m:
            self._log_iv_temp_var.set(m.group(1))
            self._log_iv_light_var.set(m.group(2))
            self._log_iv_bat_var.set(m.group(3))
            self._set_indicator('log_temp',  'flash')
            self._set_indicator('log_light', 'flash')
            self._set_indicator('log_bat',   'flash')
            if self._verify_active:
        ...
        if "cleared OK" in s:
            ...
            self._log_sv_free.set(str(self._log_total))
    ...
    def _setup_traces(self):
        ...
        for v in [self._log_iv_temp_var, self._log_iv_light_var, self._log_iv_bat_var]:
            v.trace_add('write', self._update_sensor_flash_indicators)
        for key, rv, wv in [
            ('temp',  self._temp_period_var,  self._log_iv_temp_var),
            ('light', self._light_period_var, self._log_iv_light_var),
            ('bat',   self._batt_period_var,  self._log_iv_bat_var),
        ]:
    ...
    def _update_sensor_flash_indicators(self, *_):
        for key, mode_var, mask_var, iv_var in [
            ('temp',  self._temp_mode_var,  self._log_mask_temp,  self._log_iv_temp_var),
            ('light', self._light_mode_var, self._log_mask_light, self._log_iv_light_var),
            ('bat',   self._batt_mode_var,  self._log_mask_batp,  self._log_iv_bat_var),
        ]:
    ...
    def _log_write_verify(self):
        ...
        self._verify_snapshot = {
            ...
            'temp': int(self._log_iv_temp_var.get()),
            'light': int(self._log_iv_light_var.get()),
            'bat': int(self._log_iv_bat_var.get()),
        }
        self._verify_received = {}
        self._cmd(f"log mask {mask:02X}")
        ...
        self._cmd(f"log temp {self._log_iv_temp_var.get()}")
        self._cmd(f"log light {self._log_iv_light_var.get()}")
        self._cmd(f"log bat {self._log_iv_bat_var.get()}")
    ...
    def _build_rtc_panel(self, parent):
        ...
        rtc_row = tk.Frame(rf, bg=BG)
        rtc_row.grid(row=3, column=0, columnspan=3, sticky="w", padx=2, pady=(0, 2))
        tk.Checkbutton(rtc_row, text="Live RTC  (send every 1s while UART connected)",
                       variable=self._rtc_live_var,
                       command=self._on_rtc_live_change,
                       bg=BG, fg=FG, selectcolor=BG2, activebackground=BG,
                       font=("Segoe UI", 8)
                       ).pack(side="left")
        self._ind_lbl(rtc_row, 'rtc_live').pack(side="left", padx=(8, 0))
    ...
    def _build_sched_panel(self, parent):
        ...
        sched_hdr = tk.Frame(sf, bg=BG)
        sched_hdr.grid(row=0, column=0, sticky="w", pady=(0, 3))
        tk.Checkbutton(sched_hdr, text="Enable schedule  (unchecked = TX always active)",
                       variable=self._sched_enabled,
                       command=self._toggle_sched_enabled,
                       bg=BG, fg=FG, selectcolor=BG2, activebackground=BG,
                       font=("Segoe UI", 9, "bold")
                       ).pack(side="left")
        self._ind_lbl(sched_hdr, 'sched').pack(side="left", padx=(8, 0))
    ...
    def _connect(self):
        ...
        try:
            ...
            self._save_settings()
            if self._var_silent.get():
                self.after(200, lambda: self._send_binary_cmd(0x06, bytes([1])))
    ...
    def _disconnect(self):
        if self._var_silent.get() and self.serial and self.serial.is_open:
            self._send_binary_cmd(0x06, bytes([0]))  # restore verbose before close
            time.sleep(0.05)
        self._stop_rx.set()
        time.sleep(0.1)          # let rx thread notice stop signal before port close
        if self.serial:
    ...
    def _rx_worker(self):
        ser = self.serial          # local reference — prevents AttributeError on disconnect race
        buf = b""
    ...
    def _send_binary_cmd(self, opcode: int, payload: bytes = b''):
        """Fire-and-forget machine-protocol binary command (no response wait)."""
        if not (self.serial and self.serial.is_open):
            return
        hex_str = f'{opcode:02x}' + payload.hex()
        try:
            self.serial.write(f'#1 CMD {hex_str}\r\n'.encode())
        except Exception:
            pass
    ...
    def _cmd(self, text: str):
        ...
        if not self._cmd_queue.busy:
            parts = text.strip().lower().split()
            w = parts[0] if parts else ""
            s = parts[1] if len(parts) > 1 else ""
            if w in ("status", "regs"):
                # Read-only query: reset all param indicators; _parse_status confirms each one
                for k in self._param_ind:
                    self._set_indicator(k, 'idle')
                self._last_ind_cmd = None
            elif w == "log" and s in ("get", "info", "calc", "pages", "dump"):
                # Read-only log query: reset log-config indicators; _parse_log_line confirms each
                for k in ("log_mask", "log_mode", "log_oflow", "log_ckp", "log_ts",
                          "log_temp", "log_light", "log_bat"):
                    self._set_indicator(k, 'idle')
                self._last_ind_cmd = None
            else:
                self._last_ind_cmd = text.strip()
                self._set_indicator(_cmd_to_ind(text.strip()), 'idle')
        self._log(f">>> {text}\n", "tx")
        self.serial.write((text + "\r\n").encode())
    ...
    def _do_save(self):
        ...
        cmds += self._schedule_cmds()
        cmds.append(("save", "Save to Flash"))

        # Reset all indicators to idle before sending
        for k in self._param_ind:
            self._set_indicator(k, 'idle')

        def on_done(results):
            self._show_apply_report(results)
            hard_fail = [st for c, d, st in results if st not in ('OK_FLASH', 'OK_RAM')]
            if not hard_fail:
    ...
    def _show_apply_report(self, results):
        """Scrollable report: three states — flash (green), RAM-only (grey), failed (red)."""
        flash_ok = sum(1 for _, _, s in results if s == 'OK_FLASH')
        ram_only = sum(1 for _, _, s in results if s == 'OK_RAM')
        failed_n = len(results) - flash_ok - ram_only
        ...
        if failed_n == 0 and ram_only == 0:
            hdr_text  = f"✓ All {flash_ok} settings saved to flash"
            hdr_color = "#166534"
        elif failed_n == 0:
            hdr_text  = f"◎ {flash_ok} flash, {ram_only} RAM only — explicit save handles the rest"
            hdr_color = "#92400e"
        else:
            hdr_text  = f"✗ {failed_n} FAILED  ·  {flash_ok} flash  ·  {ram_only} RAM"
            hdr_color = "#991b1b"
        ...
        # Legend
        leg = tk.Frame(win, bg=BG); leg.pack(fill="x", padx=10, pady=(0, 4))
        for sym, col, txt in [("✓","#22c55e","Saved to flash"),
                               ("◎","#94a3b8","RAM only (auto-save not seen)"),
                               ("✗","#ef4444","FAILED (ERR / TIMEOUT)")]:
            tk.Label(leg, text=sym, fg=col, bg=BG,
                     font=("Segoe UI", 9, "bold"), width=2).pack(side="left")
            tk.Label(leg, text=txt + "   ", fg=FG_DIM, bg=BG,
                     font=("Segoe UI", 8)).pack(side="left")
        ...
        for cmd, desc, status in results:
            if status == 'OK_FLASH':
                sym, col, note = "✓", "#22c55e", "flash"
            elif status == 'OK_RAM':
                sym, col, note = "◎", "#94a3b8", "RAM"
            else:
                sym, col, note = "✗", "#ef4444", status
            row = tk.Frame(inner, bg=BG2)
            row.pack(fill="x", padx=6, pady=1)
            tk.Label(row, text=sym, fg=col, bg=BG2,
            ...
            tk.Label(row, text=note, bg=BG2, fg=col,
    ...
    def _load_profiles_file(self):
        try:
            ...
            print(f"[WARN] profiles file corrupt: {e}")
        return {}
    ...
    def _load_settings(self):
        try:
            ...
            print(f"[WARN] settings file corrupt: {e}")
        return {}
    ...
    def _profile_to_gui(self, p):
        """Set all GUI variables from profile. No UART commands."""
        self._mode_var.set(p["mode"])
        self._ch_var.set(p["ch"])
        self._pwr_var.set(p["pwr"])
        self._pulse_var.set(str(p["pulse_ms"]))
        self._period_var.set(str(p["period_ms"]))
        self._led_mode_var.set(p["led_mode"])
    ...
    def _profile_to_beacon(self, p):
        """Send profile settings to MCU via CmdQueue. No-op if not connected."""
        if not (self.serial and self.serial.is_open):
            self._log("[PROFILE] not connected — GUI updated only\n", "sys")
            return
        cmds = [
            (f"mode {p['mode']}",        "TX mode"),
            (f"ch {p['ch']}",            "Channel"),
            (f"pwr {p['pwr']}",          "Power"),
            (f"pulse {p['pulse_ms']}",   "Pulse ms"),
            (f"period {p['period_ms']}", "Period ms"),
            (f"led {p['led_mode']}",     "LED mode"),
        ]
        cmds += self._schedule_cmds()
    ...
    def _on_hour_change(self):
        if getattr(self, "_sched_hour_debounce_id", None):
            self.after_cancel(self._sched_hour_debounce_id)
        self._sched_hour_debounce_id = self.after(500, self._send_sched_hours)

    def _send_sched_hours(self):
        self._sched_hour_debounce_id = None
        hours = [h for h in range(24) if self._hour_vars[h].get()]
        if hours: self._cmd("sched hours " + " ".join(str(h) for h in hours))
        else:     self._cmd("sched hours")

    def _on_day_change(self):
        if getattr(self, "_sched_day_debounce_id", None):
            self.after_cancel(self._sched_day_debounce_id)
        self._sched_day_debounce_id = self.after(500, self._send_sched_days)

    def _send_sched_days(self):
        self._sched_day_debounce_id = None
    ...
    def _on_month_change(self):
        if getattr(self, "_sched_month_debounce_id", None):
            self.after_cancel(self._sched_month_debounce_id)
        self._sched_month_debounce_id = self.after(500, self._send_sched_months)

    def _send_sched_months(self):
        self._sched_month_debounce_id = None
    ...
    def _schedule_cmds(self):
        """Return list of (cmd, desc) tuples for schedule, for use in CmdQueue."""
        ...
        months = [m+1 for m in range(12) if self._month_vars[m].get()]
        return [
            ("sched hours "  + " ".join(map(str, hours))  if hours  else "sched hours",  "Sched hours"),
            ("sched days "   + " ".join(map(str, days))   if days   else "sched days",   "Sched days"),
            ("sched months " + " ".join(map(str, months)) if months else "sched months", "Sched months"),
        ]
    ...
    def _dispatch(self, line: str):
        ...
        # Individual-command indicator (only when queue is idle)
        if self._last_ind_cmd and not self._cmd_queue.busy:
            if "[CFG] auto-saved" in s or "saved OK" in s:
                self._set_indicator(_cmd_to_ind(self._last_ind_cmd), 'flash')
                self._last_ind_cmd = None
            else:
                ack    = _ack_for_cmd(self._last_ind_cmd)
                is_err = s.startswith('[ERR]') or s.startswith('ERR')
                ack_hit = (ack != "" and ack in s) or (ack == "" and not is_err)
                if ack_hit:
                    self._set_indicator(_cmd_to_ind(self._last_ind_cmd), 'ram')
                elif is_err:
                    self._set_indicator(_cmd_to_ind(self._last_ind_cmd), 'fail')
                    self._last_ind_cmd = None
    ...
    # Map status key → indicator name (for _parse_status flash-confirm)
    _STATUS_IND = {
        'mode':         'mode',        'ch':           'ch',
        'pwr':          'pwr',         'pulse_ms':     'pulse',
        'period_ms':    'period',      'led_mode':     'led',
        'temp_mode':    'temp_mode',   'temp_period':  'temp_period',
        'temp_offset':  'temp_offset', 'batt_mode':    'batt_mode',
        'batt_period':  'batt_period', 'batt_scale':   'batt_scale',
        'light_mode':   'light_mode',  'light_period': 'light_period',
        'rtc_live':     'rtc_live',
    }
    ...
    def _parse_status(self, s: str):
        ...
        if m:
            ...
            # Confirm indicator green: value received from MCU (loaded from flash at boot)
            ind = self._STATUS_IND.get(key)
            if ind:
                self._set_indicator(ind, 'flash')
            return
    ...
    def _parse_sched_line(self, s: str):
        ...
        if m:
            ...
            self._set_indicator('sched', 'flash')
            return
