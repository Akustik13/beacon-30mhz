...
# BLE configuration opcodes (mirrors cmd_layer.h 0x70/0x71)
...
class UartTransport(Transport):
    ...
    def erase_log(self) -> Tuple[bool, str]:
        ok, lines = self._request('ERASE!', timeout=10.0)
        msg = lines[0] if lines else ('OK' if ok else 'ERR timeout')
        return ok, msg
    ...
    def update_status(self, stat: StatusBlob, info: Optional[InfoBlob] = None,
        ...
        if info:
            ...
            if total_h:
                ...
                h = stat.uptime_s // 3600
                self._d_up.setText(f'{h} h')
        else:
            h = stat.uptime_s // 3600
            self._d_up.setText(f'{h} h')
        if stat.rtc_unix:
        ...
        # Sparkline — skip invalid readings (zero or implausible for mammal implant)
        now = time.time()
        if 15.0 <= temp_c <= 55.0:   # only append credible temperatures
            self._temps.append(temp_c)
            self._temp_times.append(now)
        cutoff = now - 600
        while self._temp_times and self._temp_times[0] < cutoff:
            self._temps.pop(0)
            self._temp_times.pop(0)
        if HAS_PG and len(self._temps) > 1:
            t0 = self._temp_times[0]
            xs = [t - t0 for t in self._temp_times]
            ys = self._smooth_ma(self._temps)
            self._curve.setData(xs, ys)
            self._curve.setPen(pg.mkPen(self._C['sparkline'], width=2.5))
            # Update fill — anchored to slightly below min so area is visible
            base = min(ys) - 0.3
            self._fill.setCurves(
                self._curve,
                pg.PlotDataItem(xs, [base] * len(xs)))
            # Range label
            mn, mx = min(self._temps), max(self._temps)
            self._spark_range_lbl.setText(
                f'min {mn:.1f}°C  max {mx:.1f}°C  Δ {mx-mn:.1f}°C')
...
class DataTab(QWidget):
    ...
    def set_download_progress(self, done: int, total: int, capacity: int = 0):
        ...
        # Bar percentage = done/total (how much of THIS download is done), not done/cap
    ...
    def _replot(self, *_):
        ...
        for i, (c, cr) in enumerate(zip(self._curves, self._curves_raw)):
            ...
            if self._chk_smooth.isChecked():
                ...
                cr.setData(xs, ys); cr.setVisible(True)
...
class SettingsTab(QWidget):
    config_changed         = pyqtSignal()
    live_refresh_changed   = pyqtSignal(bool, int)   # (enabled, interval_s)
    ...
    def _build_ui(self):
        ...
        self._spin_live.setRange(1, 30); self._spin_live.setValue(2)
        self._spin_live.setSuffix(' s')
        self._spin_live.setToolTip('How often to poll STATUS from the device (1–30 s)')
        self._spin_live.setMaximumWidth(80)
        ...
        def _live_changed(*_):
            ...
            self.live_refresh_changed.emit(en, self._spin_live.value())
    ...
    def get_live_settings(self) -> tuple:
        return self._chk_live.isChecked(), self._spin_live.value()

    def set_live_settings(self, enabled: bool, interval_s: int):
        self._chk_live.blockSignals(True)
        self._spin_live.blockSignals(True)
        self._chk_live.setChecked(enabled)
        self._spin_live.setValue(max(1, min(30, interval_s)))
...
class BleTab(QWidget):
    ...
    def _build_ui(self):
        ...
        # ── Operating mode ────────────────────────────────────────────────────
        # Off and Continuous are exclusive; Schedule and Gekon can coexist.
        ...
        self._chk_gekon = QCheckBox('Gekon')
        ...
        self._chk_sched.setToolTip('Wake every N minutes, advertise for D seconds (can combine with Gekon)')
        self._chk_gekon.setToolTip('3 rapid button presses → advertising window of D seconds (can combine with Schedule)')
    ...
    def _on_off_toggled(self, checked: bool):
        if checked:
            for chk in (self._chk_cont, self._chk_sched, self._chk_gekon):
    ...
    def _on_cont_toggled(self, checked: bool):
        if checked:
            for chk in (self._chk_off, self._chk_sched, self._chk_gekon):
                chk.blockSignals(True)
                chk.setChecked(False)
                chk.blockSignals(False)
        self._update_timing_state()
    ...
    def _update_timing_state(self):
        has_timing = self._chk_sched.isChecked() or self._chk_gekon.isChecked()
    ...
    def _get_settings(self) -> BleSettings:
        if self._chk_off.isChecked():
            ...
            if self._chk_gekon.isChecked(): op_mode |= BLE_OP_GEKON
    ...
    def load_ble(self, s: BleSettings):
        bits = s.op_mode
        for chk in (self._chk_off, self._chk_cont, self._chk_sched, self._chk_gekon):
            chk.blockSignals(True)
        if bits == BLE_OP_OFF:
            self._chk_off.setChecked(True)
            self._chk_cont.setChecked(False)
            self._chk_sched.setChecked(False)
            self._chk_gekon.setChecked(False)
            ...
            self._chk_gekon.setChecked(bool(bits & BLE_OP_GEKON))
        for chk in (self._chk_off, self._chk_cont, self._chk_sched, self._chk_gekon):
            chk.blockSignals(False)
...
class MainWindow(QMainWindow):
    def __init__(self):
        ...
        self._refresh_timer.setInterval(live_iv * 1000 if live_en else 5000)
    ...
    def _load_settings(self):
        try:
            if os.path.exists(self._settings_file):
                ...
                self._live_iv        = d.get('live_refresh_s',  2)
    ...
    def _build_top_bar(self) -> QWidget:
        ...
        self._combo_src.setFixedWidth(160)
    ...
    def _do_connect(self):
        ...
        if isinstance(t, MockBleTransport):
            t.connect_to('')
    ...
    def _update_connection_ui(self, connected: bool):
        ...
        if connected:
            src = self._combo_src.currentText()
            name = src.replace('Serial ', '')
            self._dot.setStyleSheet(f'color:{C["success"]};')
            self._lbl_conn.setText(f'Connected  {name}')
    ...
    def _init_after_connect(self):
        """Read INFO, CFG, HwDesc, sensor list after connect then go to Overview."""
        def _read():
            t = self._transport
            info_b  = t.read_info()
            cfg_b   = t.read_config()
            ...
            return info_b, cfg_b, stat_b, hwdesc, sensors
    ...
    def _on_init_done(self, result):
        ...
        info_b, cfg_b, stat_b, hwdesc, sensors = result
        if len(info_b) == 48:
    ...
    def _on_live_refresh_changed(self, enabled: bool, iv_s: int):
        self._refresh_timer.setInterval(iv_s * 1000 if enabled else 5000)
        self._tab_overview.set_live_streaming(enabled and self._connected)
        self._save_settings()
        if enabled:
            self._banner.show_ok(f'Live refresh: every {iv_s} s — saved', 2000)
        else:
    ...
    def _on_apply(self):
        ...
        def _do_apply():
            ...
            # Save main config to flash
            s_ok, s_msg = self._transport.save_flash()
            # Send accel enable/interval (saved to log header page by firmware)
            t = self._transport
            ...
            return True, 'All settings applied, verified and saved to flash'
    ...
    def _erase_log(self):
        if not self._connected or not self._transport:
            self._banner.show_err('Not connected')
            return
        self._refresh_timer.stop()  # avoid UART race during flash erase
    ...
    def _on_ble_apply(self, s):
        ...
        def _do():
            ...
            if rc == CMD_OK:
                ...
                _time.sleep(0.15)          # let main loop flush config to flash
                try:
                    self._transport.cmd_reboot()
                except Exception:
                    pass  # expected — MCU resets USB before RSP arrives
            return rc
        ...
        def _done(rc):
            if rc == CMD_OK:
                self._tab_ble.set_status('Saved — beacon rebooting…', ok=True)
                self._banner.show_ok('BLE settings saved — reconnect in ~3 s', 5000)
                ...
                QTimer.singleShot(150, self._do_disconnect)  # disconnect before MCU resets
            else:
