...
void Custom_STM_App_Notification(Custom_STM_App_Notification_evt_t *pNotification)
{
  ...
  switch (pNotification->Custom_Evt_Opcode)
  {
    ...
    case CUSTOM_STM_MOTION_C_NOTIFY_ENABLED_EVT:
      ...
      /* MOTION_C stream intentionally disabled — accel data sent via ENV_C only */
...
void Custom_APP_Notification(Custom_App_ConnHandle_Not_evt_t *pNotification)
{
  ...
  switch (pNotification->Custom_Evt_Opcode)
  {
    ...
    case CUSTOM_CONN_HANDLE_EVT :
      ...
      APP_DBG_MSG("-- BLE connected, handle=0x%04X\n", Connection_Handle);
      /* Push ENV_C immediately so client can READ it before subscribing */
      ENV_Update();
...
void Custom_APP_Init(void)
{
  ...
  /* Removed: HRS_SL handle is invalid (leftover My_HRS service from HR template) → 0x60 error */
