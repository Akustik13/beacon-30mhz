...
/* USER CODE BEGIN PD */
#define BLE_LED_ADV_BLINK_PERIOD      (uint32_t)(0.5*1000*1000/CFG_TS_TICK_VAL)  /* LED toggle each 500ms -> 1s blink */
#define BLE_LED_FAST_BLINK_PERIOD     (uint32_t)(0.1*1000*1000/CFG_TS_TICK_VAL)  /* 5 fast blinks after disconnect */
#define BLE_DISCONN_FAST_TOGGLES      10U                                      /* 5 on/off blinks */
/* USER CODE END PD */
 ...
 * Advertising Data
 */
uint8_t a_AdvData[30] =
{
  /* length=9: type(1) + "WB1M"(4) + last-2-MAC-bytes as hex(4) — filled by fill_advData */
  /* "BCN_XXXX" — last 4 hex chars of MAC filled by fill_advData at runtime */
  9, AD_TYPE_COMPLETE_LOCAL_NAME, 'B', 'C', 'N', '_', 0x30, 0x30, 0x30, 0x30,
...
/* USER CODE BEGIN PV */
static uint8_t Ble_Led_Timer_Id;
static uint8_t Ble_Led_FastToggleCounter;
volatile uint8_t g_ble_advertising = 0;
...
static void BLE_Led_Timer_Callback(void);
static void BLE_Led_Start_Advertising_Blink(void);
static void BLE_Led_Start_Disconnect_Blink(void);
static void BLE_Led_Set_Connected(void);
/* USER CODE END PFP */
...
void APP_BLE_Init(void)
{
  ...
  HW_TS_Create(CFG_TIM_PROC_ID_ISR, &(Ble_Led_Timer_Id), hw_ts_Repeated, BLE_Led_Timer_Callback);
  ...
  /* LED stays OFF after init — advertising NOT auto-started.
     BLE_Led_Start_Advertising_Blink() is called from Adv_Request() when advertising actually begins. */
  BSP_LED_Off(LED_BLUE);
  ...
  /* Advertising NOT started at init — press B1 button to start LP advertising */
  BleApplicationContext.Device_Connection_Status = APP_BLE_IDLE;

  /* USER CODE BEGIN APP_BLE_Init_2 */
  /* Apply BLE TX power from config */
  aci_hal_set_tx_power_level(1, g_config.ble_tx_power);
  printf("[BLE] TX power PA_Level=%u\r\n", (unsigned)g_config.ble_tx_power);
  /* Advertising NOT auto-started — send BLESTART command to begin */
  printf("[BLE] BLE stack ready — send BLESTART to advertise\r\n");
  ...
  switch (p_event_pckt->evt)
  {
    case HCI_DISCONNECTION_COMPLETE_EVT_CODE:
    {
      ...
      if (p_disconnection_complete_event->Connection_Handle == BleApplicationContext.BleApplicationContext_legacy.connectionHandle)
      {
        ...
        g_ble_connected   = 0;
        g_ble_advertising = 0;
        printf("[BLE] Disconnected — restarting advertising as %.8s\r\n", (char*)&a_AdvData[2]);
      ...
      BLE_Led_Start_Disconnect_Blink();
      /* USER CODE END EVT_DISCONN_COMPLETE_1 */

      /* No auto-restart — press B1 to advertise again */
      BleApplicationContext.Device_Connection_Status = APP_BLE_IDLE;
    ...
    case HCI_VENDOR_SPECIFIC_DEBUG_EVT_CODE:
      ...
      switch (p_blecore_evt->ecode)
      {
        ...
        case ACI_HAL_END_OF_RADIO_ACTIVITY_VSEVT_CODE:
          ...
          /* LED blinking is handled by BLE_Led_Timer_Callback(). */
...
/* USER CODE END Role_Mngt */
  ...
  /* Update GATT Device Name char with dynamic BCN_XXXX so scanner sees it as device name */
  ret = aci_gatt_update_char_value(gap_service_handle, gap_dev_name_char_handle, 0, 8, &a_AdvData[2]);
  APP_DBG_MSG("[BLE] Adv name: %.8s  GATT DevName: %s\r\n",
              (char*)&a_AdvData[2], ret == BLE_STATUS_SUCCESS ? "OK" : "FAIL");
...
static void Adv_Request(APP_BLE_ConnStatus_t NewStatus)
{
  ...
  /* interval from config: 1 unit = 0.625ms → units = ms * 8 / 5 */
  uint16_t adv_units = (uint16_t)((g_config.ble_adv_ms * 8U) / 5U);
  if (adv_units < 0x0020) adv_units = 0x0020; /* min 20ms per BT spec */
  ...
  if (ret != BLE_STATUS_SUCCESS)
  {
    ...
    printf("[BLE] aci_gap_set_discoverable FAIL ret=0x%02X\r\n", (unsigned)ret);
    BleApplicationContext.Device_Connection_Status = APP_BLE_IDLE; /* reset status on fail */
    return;
  }
  else
  {
    APP_DBG_MSG("==>> aci_gap_set_discoverable - Success\n");
    /* Advertising is live from this point — start LED blink regardless of adv data update */
    g_ble_advertising = 1;
    g_ble_connected   = 0;
    BLE_Led_Start_Advertising_Blink();
  }
...
/* USER CODE END Adv_Request_1*/
  ...
  /* Update advertising name/data — decorative; advertising runs even if this fails */
  ret = aci_gap_update_adv_data(sizeof(a_AdvData), (uint8_t*) a_AdvData);
  if (ret != BLE_STATUS_SUCCESS)
  {
      APP_DBG_MSG("==>> Start Fast Advertising Failed , result: %d \n\r", ret);
      printf("[BLE] aci_gap_update_adv_data FAIL ret=0x%02X\r\n", (unsigned)ret);
  }
  else
  {
      APP_DBG_MSG("==>> Success: Start Fast Advertising \n\r");
      printf("[BLE] Advertising as %.8s (adv=%lums) — scan now\r\n",
             (char*)&a_AdvData[2], (unsigned long)g_config.ble_adv_ms);
  }
...
static void BLE_Led_Timer_Callback(void)
{
  /* Only drive LED from this timer in BLE_STATUS mode.
     In other modes (OFF, TX_PULSE, HEARTBEAT, ALWAYS_ON) the main loop or
     rf_enable() owns the LED — stop timer and leave LED alone. */
  if (g_config.led_mode != LED_BLE_STATUS) {
    Ble_Led_FastToggleCounter = 0U;
    HW_TS_Stop(Ble_Led_Timer_Id);
    return;
  }

  if (Ble_Led_FastToggleCounter > 0U)
  {
    BSP_LED_Toggle(LED_BLUE);
    Ble_Led_FastToggleCounter--;
...
static void BLE_Led_Start_Advertising_Blink(void)
{
  Ble_Led_FastToggleCounter = 0U;
  HW_TS_Stop(Ble_Led_Timer_Id);
  if (g_config.led_mode != LED_BLE_STATUS) return;
  BSP_LED_Off(LED_BLUE);
  HW_TS_Start(Ble_Led_Timer_Id, BLE_LED_ADV_BLINK_PERIOD);
}
...
static void BLE_Led_Start_Disconnect_Blink(void)
{
  HW_TS_Stop(Ble_Led_Timer_Id);
  if (g_config.led_mode != LED_BLE_STATUS) return;
  Ble_Led_FastToggleCounter = BLE_DISCONN_FAST_TOGGLES;
  BSP_LED_Off(LED_BLUE);
  HW_TS_Start(Ble_Led_Timer_Id, BLE_LED_FAST_BLINK_PERIOD);
}
...
static void fill_advData(uint8_t *p_adv_data, uint8_t tab_size, const uint8_t* p_bd_addr)
{
  ...
  while(i < tab_size)
  {
    ...
    case AD_TYPE_COMPLETE_LOCAL_NAME:
      {
        static const char bcn_hex[] = "0123456789ABCDEF";
        p_adv_data[i+6] = bcn_hex[(p_bd_addr[1] >> 4) & 0x0F];
        p_adv_data[i+7] = bcn_hex[p_bd_addr[1] & 0x0F];
        p_adv_data[i+8] = bcn_hex[(p_bd_addr[0] >> 4) & 0x0F];
        p_adv_data[i+9] = bcn_hex[p_bd_addr[0] & 0x0F];
        break;
      }
...
{
  /* Ignore spurious EXTI from APPE_Button_Init() firing before BLE stack is up */
  if (g_cpu2_state == 0) return;

  /* B1 press: toggle advertising */
  if (BleApplicationContext.Device_Connection_Status == APP_BLE_IDLE) {
    APP_DBG_MSG("B1: start LP advertising\r\n");
    Adv_Request(APP_BLE_LP_ADV);
  } else if (BleApplicationContext.Device_Connection_Status == APP_BLE_LP_ADV ||
             BleApplicationContext.Device_Connection_Status == APP_BLE_FAST_ADV) {
    APP_DBG_MSG("B1: stop advertising\r\n");
    Adv_Cancel();
  }
}

/* Restart advertising with current g_config.ble_adv_ms.
   Safe to call from Uart_Cmd_Task: aci_gap_set_non_discoverable and
   aci_gap_set_discoverable use UTIL_SEQ_WaitEvt which spins via __WFI
   and processes IPCC interrupts normally inside a UTIL_SEQ task. */
void APP_BLE_Adv_Restart(void)
{
  if (g_cpu2_state == 0) return;
  APP_BLE_ConnStatus_t st = BleApplicationContext.Device_Connection_Status;
  printf("[BLE] Adv_Restart: st=%u\r\n", (unsigned)st);
  if (st == APP_BLE_LP_ADV || st == APP_BLE_FAST_ADV) {
    tBleStatus r = aci_gap_set_non_discoverable();
    printf("[BLE] non_disc ret=0x%02X\r\n", (unsigned)r);
    BleApplicationContext.Device_Connection_Status = APP_BLE_IDLE;
    g_ble_advertising = 0;
  }
  if (BleApplicationContext.Device_Connection_Status == APP_BLE_IDLE) {
...
{
  if (g_cpu2_state == 0) { printf("[BLE] Adv_Stop: CPU2 not ready\r\n"); return; }
  APP_BLE_ConnStatus_t st = BleApplicationContext.Device_Connection_Status;
  printf("[BLE] Adv_Stop: st=%u adv=%u\r\n", (unsigned)st, (unsigned)g_ble_advertising);
  if (st != APP_BLE_IDLE && st != APP_BLE_CONNECTED_SERVER) {
    tBleStatus r = aci_gap_set_non_discoverable();
    BleApplicationContext.Device_Connection_Status = APP_BLE_IDLE;
    g_ble_advertising = 0;
    printf("[BLE] non_disc ret=0x%02X -> IDLE\r\n", (unsigned)r);
    if (g_config.led_mode == LED_BLE_STATUS) {
      HW_TS_Stop(Ble_Led_Timer_Id);
      BSP_LED_Off(LED_BLUE);
    }
  } else {
    printf("[BLE] already IDLE/CONNECTED — nothing to stop\r\n");
  }
}
